...
REM endlocal & bash --login && cd "/usr/bin" && pwd && "/usr/bin/mygrep.sh" %*
REM endlocal & echo cd /cygdrive/c/cbn_gitsﾍｾ "%_CYGSCRIPT%" %* | bash --login -s
REM echo "/usr/bin/mygrep.sh" %* |

REM endlocal & echo "%_CYGSCRIPT%" %*| %_CYGBIN%\bash --login -s
echo pwd; cd "/usr/bin/"; pwd ; "/usr/bin/mygrep.sh" %*| "c:\cygwin64\bin\bash" --login -s
