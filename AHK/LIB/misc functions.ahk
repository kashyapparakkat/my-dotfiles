...
; TODO make it work for paths like THISPC>Downloads
get_parent_filepath()
{
...
get_current_filepath_else_sourcepath_from_active_window()
{
...
ifNotExist,%sourcepath%
	...
	run,"%f_path%" --eval "%eval_expression%" -n
	WinActivate,ahk_exe emacs.exe

}
run_emacs_dired(sourcepath)
{
	stringreplace,sourcepath,sourcepath,\,/,all
	eval_expression=(progn (print default-directory)(dired \"%sourcepath%\"))
	iniread,f_path,C:\cbn_gits\AHK\running_all_settings.ini,paths,emacs
	run,"%f_path%" --eval "%eval_expression%" -n
	WinActivate,ahk_exe emacs.exe
