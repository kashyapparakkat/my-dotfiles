...
; disabling for sometime
; <^f11::	; shows GUI - paste Selected_Text line by line
 ...
 CLIPSTEP2:
{
	...
	If state=u
	{
	  ...
	  Else If paste2=paste
	  {
		...
		t:=no_of_lines(readclip)
		t2:=strlen(readclip)
		tooltip=Pasting clip #%MCF_activeclip% (%t2% %t%)`n%MyText%
		Gosub,TOOLTIP
		Gosub,PASTECLIP2
...
CLIPSTEP:
{
	...
	If state=u
	{
	  ...
	  Else If paste=paste
	  {
		...
		t:=no_of_lines(readclip)
		t2:=strlen(readclip)
		tooltip=Pasting clip #%activeclip% (%t2% %t%)`n%MyText%
		Gosub,TOOLTIP
		Gosub,PASTECLIP
		ClipStep_Keys:=0
		activeclip=1
...
#include C:\cbn_gits\AHK\paste_collector\Gdip_All.ahk
