...
if [ -z "$1" ]; then
	...
	echo '(' > "$file"
#	find.exe /cygdrive/c/cbn_gits/AHK/LIB  -iname "*.ahk" -printf '("%P" "%h")\n'  | sed -r "s/\\/cygdrive\\/(.)\\//\1:\//" | tr -d '\n' >> "$file"
	find  /cygdrive/c/cbn_gits/AHK/*  $Universal_home/Downloads/* -iregex ".*\.\(txt\|py\|ini\|java\|ahk\)" -printf '("%f" "%h")\n'  | sed -r "s/\\/cygdrive\\/(.)\\//\1:\//" | tr -d '\n' >> "$file"
	echo ')' >> "$file"
fi
 }
...
function myindex() { 
	...
	if [ "$machine" == "Windows" ]; then 
		...
		find /cygdrive -iname "*"  > ~/"all_files_unfiltered.db"
